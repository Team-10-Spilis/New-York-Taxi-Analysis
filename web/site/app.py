...
st.markdown("""
    <style>
    /* Скрываем кружки только у первой панели радиокнопок (навигации) */
    div[data-testid="stRadio"]:first-of-type div[role="radiogroup"] > label > div:first-child { display: none; }
    
    /* Стилизуем первую панель под вкладки */
    ...
    /* Выделяем активную вкладку цветом */
...
st.title("🚖 Анализ поездок такси в Нью-Йорке")
...
def load_data():
    ...
    # ИСПРАВЛЕНО 2 И 4: Убираем левые районы и берем поездки не старше 2023 года
    df = df[~df['PU_Borough'].isin(['EWR', 'Unknown'])]
    df = df[~df['DO_Borough'].isin(['EWR', 'Unknown'])]
...
VENDOR_ID_LABELS = {
    "Неизвестно": 2,
    "Creative Mobile Technologies (CMT)": 1,
    "VeriFone / VTS": 2,
}
...
RATECODE_ID_LABELS = {
    ...
    "Тариф JFK Airport": 2,
    "Тариф Newark": 3,
    "Тариф Nassau / Westchester": 4,
    "Договорная стоимость": 5,
    "Групповая поездка": 6,
}
...
WEATHER_INPUT_DEFAULTS = {
    "Неизвестно": (0.0, 0.0),
    "Ясно": (0.0, 0.0),
    "Преимущественно ясно": (0.0, 0.0),
    "Переменная облачность": (0.0, 0.0),
    "Пасмурно": (0.0, 0.0),
    "Туман": (0.0, 0.0),
    "Изморозь / ледяной туман": (0.0, 0.1),
    "Слабая морось": (0.2, 0.0),
    "Морось": (0.5, 0.0),
    "Сильная морось": (1.0, 0.0),
    "Небольшой дождь": (1.0, 0.0),
    "Дождь": (3.0, 0.0),
    "Сильный дождь": (8.0, 0.0),
    "Небольшой снег": (0.2, 0.5),
    "Снег": (0.5, 1.5),
    "Сильный снег": (1.0, 4.0),
    "Ливень": (5.0, 0.0),
    "Сильный ливень": (12.0, 0.0),
    "Гроза": (8.0, 0.0),
}

TEMPERATURE_UNKNOWN_VALUE = 15.0
...
def render_ml_predictions_tab(df):
    ...
    if use_zone_dropdowns:
        ...
        with col4:
            pu_query = st.text_input("Поиск зоны посадки", value="", key="ml_pu_zone_search")
            pu_labels = [label for label in labels if pu_query.casefold() in label.casefold()]
            if not pu_labels:
                st.caption("По запросу ничего не найдено, показаны все зоны.")
                pu_labels = labels
            default_pu_label = labels[default_pu] if labels else None
            pu_index = pu_labels.index(default_pu_label) if default_pu_label in pu_labels else 0
            pu_label = st.selectbox("Зона посадки", pu_labels, index=pu_index, key="ml_pu_zone")
        with col5:
            do_query = st.text_input("Поиск зоны высадки", value="", key="ml_do_zone_search")
            do_labels = [label for label in labels if do_query.casefold() in label.casefold()]
            if not do_labels:
                st.caption("По запросу ничего не найдено, показаны все зоны.")
                do_labels = labels
            default_do_label = labels[default_do] if labels else None
            do_index = do_labels.index(default_do_label) if default_do_label in do_labels else 0
            do_label = st.selectbox("Зона высадки", do_labels, index=do_index, key="ml_do_zone")
    ...
    with col6:
        trip_distance = min(100.0, max(0.1, round(float(gps_distance) * 1.25, 2)))
        st.number_input("Дистанция поездки, мили", min_value=0.1, max_value=100.0, value=float(trip_distance), step=0.1, disabled=True)
    with col7:
        vendor_label = st.selectbox("Поставщик таксометра", list(VENDOR_ID_LABELS.keys()), index=0)
        vendor_id = VENDOR_ID_LABELS[vendor_label]
    with col8:
        ratecode_label = st.selectbox("Тип тарифа", list(RATECODE_ID_LABELS.keys()), index=0)
        ratecode_id = RATECODE_ID_LABELS[ratecode_label]
    with col9:
        payment_label = st.selectbox("Способ оплаты", list(PAYMENT_TYPE_LABELS.keys()), index=0)
        payment_type = PAYMENT_TYPE_LABELS[payment_label]

    col10, col11 = st.columns(2)
    with col10:
        temperature_mode = st.selectbox("Температура", ["Неизвестно", "Указать вручную"], index=0)
        if temperature_mode == "Указать вручную":
            temperature = st.number_input("Значение температуры, °C", min_value=-30.0, max_value=50.0, value=15.0, step=0.5)
        else:
            temperature = TEMPERATURE_UNKNOWN_VALUE
    with col11:
        weather_label = st.selectbox("Погода", list(WEATHER_CODE_LABELS.keys()), index=0)
        weather_code = WEATHER_CODE_LABELS[weather_label]
        precipitation, snowfall = WEATHER_INPUT_DEFAULTS.get(weather_label, WEATHER_INPUT_DEFAULTS["Неизвестно"])
        precipitation = float(precipitation)
        snowfall = float(snowfall)

    with st.expander("Исторические признаки для прогноза спроса и выручки"):
...
if selected_tab == "🗺️ 3D Карта потоков":
    ...
    # ИСПРАВЛЕНО 2: Иерархический фильтр для районов и зон
    ...
    if selected_pu_boroughs:
        pu_zones = sorted(data[data['PU_Borough'].isin(selected_pu_boroughs)]['PU_Zone'].dropna().unique())
    else:
        pu_zones = sorted(data['PU_Zone'].dropna().unique())
    selected_pu_zones = st.sidebar.multiselect("Зона выезда (Откуда - части)", pu_zones, placeholder="Все зоны")
    
    selected_do_boroughs = st.sidebar.multiselect("Район въезда (Куда)", all_boroughs, placeholder="Все районы")
    if selected_do_boroughs:
        do_zones = sorted(data[data['DO_Borough'].isin(selected_do_boroughs)]['DO_Zone'].dropna().unique())
    else:
        do_zones = sorted(data['DO_Zone'].dropna().unique())
    selected_do_zones = st.sidebar.multiselect("Зона въезда (Куда - части)", do_zones, placeholder="Все зоны")
...
if selected_pu_zones:
    filtered_data = filtered_data[filtered_data['PU_Zone'].isin(selected_pu_zones)]
if selected_do_boroughs:
    filtered_data = filtered_data[filtered_data['DO_Borough'].isin(selected_do_boroughs)]
if selected_do_zones:
    filtered_data = filtered_data[filtered_data['DO_Zone'].isin(selected_do_zones)]
...
if selected_tab == "🗺️ 3D Карта потоков":
    ...
    heatmap_type = st.radio("Что анализируем?", ["Посадки (Спрос)", "Высадки (Притяжение)"], horizontal=True, label_visibility="collapsed")
    ...
    with col1:
        ...
        if not top_trips.empty:
            ...
            st.plotly_chart(px.bar(top_trips, x='trip_count', y='route', orientation='h', title="Самые частые маршруты").update_layout(yaxis={'categoryorder':'total ascending'}), use_container_width=True)
    ...
    with col2:
        ...
        if not popular_routes.empty:
            ...
            st.plotly_chart(px.bar(top_revenue, x='avg_fare', y='route', orientation='h', title="Самый высокий средний чек ($)", color_discrete_sequence=['#2ca02c']).update_layout(yaxis={'categoryorder':'total ascending'}), use_container_width=True)
    ...
    with col4:
        ...
        st.plotly_chart(px.line(hourly_demand, x='pickup_hour', y='trips', markers=True, title="Общий спрос по часам (Загруженность)", labels={'pickup_hour': 'Час дня', 'trips': 'Поездок'}), use_container_width=True)
    ...
    with col5:
        ...
        st.plotly_chart(px.bar(fpm_df.sort_values('fare_per_mile', ascending=False), x='PU_Borough', y='fare_per_mile', title="Рентабельность ($ за милю) по районам", labels={'PU_Borough': 'Район', 'fare_per_mile': '$ за милю'}, color='fare_per_mile', color_continuous_scale="Greens"), use_container_width=True)
    ...
    with col6:
        ...
        st.plotly_chart(px.scatter(scatter_sample, x='trip_distance', y='fare_amount', color='PU_Borough', title="Стоимость vs Дистанция (Выборка 2000 поездок)", labels={'trip_distance': 'Дистанция (миль)', 'fare_amount': 'Тариф ($)'}, opacity=0.6), use_container_width=True)
    ...
    with col7:
        ...
        st.plotly_chart(px.pie(pass_df, values='Количество', names='Пассажиры', hole=0.4, title="Структура поездок по числу пассажиров"), use_container_width=True)

    with col8:
        # ИСПРАВЛЕНО 4: График дистанций больше не съезжает в даты!
        ...
        bins_labels = [f"{int(bins[i])} - {int(bins[i+1])} миль" for i in range(len(bins)-1)]
        dist_hist_df = pd.DataFrame({'Дистанция (мили)': bins_labels, 'Количество': counts})
        
        fig_hist = px.bar(dist_hist_df, x='Дистанция (мили)', y='Количество', title="Какую дистанцию чаще всего ездят?", color_discrete_sequence=['#1f77b4'])
        fig_hist.update_xaxes(type='category') # Явно говорим Plotly не превращать числа в даты!
    ...
    if zones_geojson is None:
        ...
        with col_c1:
            if not os.path.exists("clusters.csv"):
                st.warning("⚠️ Файл `clusters.csv` не найден! Показаны случайные ДЕМО-ДАННЫЕ.")
                
            st.markdown("#### 📂 Датасет кластеров (Срез)")
            
            # ИСПРАВЛЕНО 6: Переводим технические ID в красивые человеческие названия
            readable_clusters = clusters_df.copy()
            zone_mapping = data[['PULocationID', 'PU_Borough', 'PU_Zone']].drop_duplicates().set_index('PULocationID')
            readable_clusters = readable_clusters.join(zone_mapping, on='PULocationID')
            readable_clusters = readable_clusters.dropna()
            
            readable_clusters = readable_clusters.rename(columns={
                'PULocationID': 'ID Локации',
                'PU_Borough': 'Район (Borough)',
                'PU_Zone': 'Округ (Zone)',
                'cluster_id': 'Номер кластера'
            })
            
            st.dataframe(readable_clusters[['ID Локации', 'Район (Borough)', 'Округ (Zone)', 'Номер кластера']], use_container_width=True)
        ...
        with col_c2:
            ...
            st.plotly_chart(px.bar(cluster_counts, x="Кластер", y="Количество зон", color="Кластер", color_discrete_sequence=['#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00', '#00bfbf', '#ffd700', '#ff1493']), use_container_width=True)